...
public class IconGenerator {
     ...
     *Ahmed Ibrahim
     * @param text the text content to display inside the icon.
     * @param height the specified height the icon.
     * @param width the specified width the icon.
     */
    public Bitmap makeIcon(String text, int height, int width) {
        if (mTextView != null) {
            mTextView.setText(text);
            mTextView.setPadding(20,2,20,2);
        }

        return makeIcon(height,  width);
    }
    ...
    /**
     * Creates an icon with the current content and style by specifying the width and height -- Ahmed Ibrahim.
     * <p/>
     * This method is useful if a custom view has previously been set, or if text content is not
     * applicable.
     */
    public Bitmap makeIcon(int height, int width) {
        int measureSpec = View.MeasureSpec.makeMeasureSpec(0, View.MeasureSpec.UNSPECIFIED);
        mContainer.measure(measureSpec, measureSpec);
